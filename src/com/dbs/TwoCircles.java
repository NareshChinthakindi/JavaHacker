...
public class TwoCircles {
	...
	public static void main(String[] args) {
		...
		/*
		 * 4 12 0 21 14 0 23 0 45 8 0 94 9 35 0 13 10 0 38 0 26 8 0 9 25
		 */
	...
	public static void readTestData(Scanner scanner, List<int[]> testsList) {
		// c1x (c1's center x axis value),c1y (c1's center y axis value),c1R
		// (c1's radius)
		// c2x (c2's center x axis value),c2y (c2's center y axis value),c2R
		// (c2's radius)
	...
	public static boolean isDisjoinInside(int c1CenterPoint, int c1Radius, int c2CenterPoint, int c2Radius) {
		...
		if (isC1CenterLessThanC2Center) {
			...
			// if c1 contains c2, then c2 boundaries should be inside c1
			// boundaries
			if (((c2CenterPoint + c2Radius) < (c1CenterPoint + c1Radius))
					&& ((c2CenterPoint - c2Radius) > (c1CenterPoint + c1Radius))) {
				return true;

			} else if (((c2CenterPoint - c2Radius) < (c1CenterPoint - c1Radius))
					&& ((c2CenterPoint + c2Radius) > (c1CenterPoint + c1Radius))) {
				//if c2 contains c1, then c1 boundaries should be inside c2 boundary
